...
public class FatherAdapter extends RecyclerView.Adapter<FatherAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        Father father = fathers.get(position);
        holder.tv_id.setText("父id: " + father.getId() + "子长度： " + father.getSonList().size());
        holder.tv_name.setText("父name: " + father.getFatherName());

        holder.tv_name.setOnClickListener(v -> {
            if (onItemClickListener != null && father.getSonList().size() != 0){
   ...
   public interface OnItemClickListener{
        void onItemClick(Father father);
    }
}
